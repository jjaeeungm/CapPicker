...
namespace CapPicker
{
    ...
    internal sealed class SelectionOverlay : Form
    {
        ...
        private void CaptureScreenPreview()
        {
            ...
            // 가벼움 완화: 일반 PC는 풀 프리뷰(빠름·편함) 유지.
            // 저사양 모드 + 초대형 가상화면(24MP 초과, 4K 3대≈24.9MP)에서만
            // 프리뷰 비트맵(약 96MB+)을 생략하고 어두운 오버레이로 동작 (MuseSpark).
            long pixels = (long)Math.Max(1, v.Width) * Math.Max(1, v.Height);
            if (AppSettings.LowSpecOptimization && pixels > 24000000L)
            {
