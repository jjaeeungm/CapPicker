...
namespace CapPicker
{
    internal sealed class HelpForm : Form
    {
        ...
        public HelpForm()
        {
            ...
            // 폰트는 RefreshHelpPreview → BuildHelpDocument에서 언어별로 설정 (중복 생성 방지).
        ...
        {
            // 자식 컨트롤은 base가 처리. 직접 만든 GDI 폰트만 명시 해제 (MuseSpark).
            Font ownedFormFont = null;
            Font ownedTextFont = null;
            if (disposing)
            {
                ownedFormFont = Font;
                if (helpText != null) ownedTextFont = helpText.Font;
            }
            ...
            if (disposing)
            {
                try { if (ownedTextFont != null && !ReferenceEquals(ownedTextFont, ownedFormFont)) ownedTextFont.Dispose(); } catch { }
                try { if (ownedFormFont != null) ownedFormFont.Dispose(); } catch { }
            }
        }
