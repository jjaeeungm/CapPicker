...
namespace CapPicker
{
    ...
    internal sealed class EditorCanvas : Control
    {
        ...
        private const long MaxHistoryBytes = 100L * 1024L * 1024L;
        ...
        private void DrawImageSampleMagnifier(Graphics g)
        {
            ...
            try
            {
                ...
                // 가벼움: GetPixel(cells^2회) 대신 필요 영역만 LockBits 1회 복사 (MuseSpark).
                Rectangle need = Rectangle.FromLTRB(startX, startY, startX + cells, startY + cells);
                need.Intersect(new Rectangle(0, 0, current.Width, current.Height));
                int[] fastPixels = null;
                int fastStride = 0;
                int fastLeft = need.Left;
                int fastTop = need.Top;
                BitmapData fastData = null;
                try
                {
                    if (need.Width > 0 && need.Height > 0)
                    {
                        fastData = current.LockBits(need, ImageLockMode.ReadOnly, PixelFormat.Format32bppArgb);
                        fastStride = Math.Abs(fastData.Stride) / 4;
                        fastPixels = new int[fastStride * need.Height];
                        Marshal.Copy(fastData.Scan0, fastPixels, 0, fastPixels.Length);
                    }
                }
                catch { fastPixels = null; }
                finally
                {
                    try { if (fastData != null) current.UnlockBits(fastData); } catch { }
                }
                ...
                using (SolidBrush pixelBrush = new SolidBrush(Color.Transparent))
                {
                    for (int row = 0; row < cells; row++)
                    {
                        for (int col = 0; col < cells; col++)
                        {
                            ...
                            {
                                Color px;
                                if (fastPixels != null && sx >= fastLeft && sx < fastLeft + need.Width &&
                                    sy >= fastTop && sy < fastTop + need.Height)
                                {
                                    int argb = fastPixels[(sy - fastTop) * fastStride + (sx - fastLeft)];
                                    px = Color.FromArgb(argb);
                                }
                                else
                                {
                                    px = current.GetPixel(sx, sy);
                                }
                                pixelBrush.Color = px;
        ...
        private void SaveUndo()
        {
            ...
            {
                // OOM/GDI+ 실패 시 히스토리를 건너뛰고 그리기 자체는 유지 (MuseSpark 보완).
                try { while (undo.Count > 0) undo.Pop(); } catch { }
            }
        }

        private void TrimHistoryBytes()
        {
            try
            {
                while ((StackBytes(undo) + StackBytes(redo)) > MaxHistoryBytes && undo.Count > 1)
                    DropOldest(undo);
                while (StackBytes(redo) > MaxHistoryBytes && redo.Count > 1)
                    DropOldest(redo);
            }
            catch { }
        }

        private static long StateBytes(EditorState s)
        {
            if (s == null) return 0;
            long n = 0;
            if (s.Current != null) n += s.Current.Length;
            if (s.Clean != null) n += s.Clean.Length;
            return n;
        }

        private static long StackBytes(Stack<EditorState> stack)
        {
            if (stack == null || stack.Count == 0) return 0;
            long n = 0;
            foreach (EditorState s in stack) n += StateBytes(s);
            return n;
        }

        private static void DropOldest(Stack<EditorState> stack)
        {
